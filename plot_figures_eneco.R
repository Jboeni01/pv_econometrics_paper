...
load(paste0(getwd(),"/tariffs_pel_subreg.Rdata")) #subregional electricity prices
load(paste0(getwd(),"/npv_final.Rdata")) #npv data 
load(paste0(getwd(),"/b_subreg.Rdata")) #npv data for subregions 
...
benefitsdr <- paste0(c("b_gc","b_nm","b_protar","b_tc","b_qw"),dr)
benefitsdr_defl <- paste0(benefitsdr,"_defl")
...
npv_df <- npv_final %>% 
  rename_with(~ gsub("[0-9]+", "", .x), .cols = all_of(c(benefitsdr, benefitsdr_defl))) %>%
  rowwise() %>%
  mutate(net_benefits = sum(c_across(all_of(benefits))),
         net_benefits_defl = sum(c_across(all_of(benefits_defl)))) %>%
  ungroup() %>%
  mutate(npv = net_benefits - get(c_capkw),
         npv_defl = net_benefits_defl - get(c_capkw_defl)) %>%
...
col_lab <- c("tax credit","output-based incentive","capacity-based incentive","net metering","capacity-based cost")
...
npv_yrange_list <- list("yrange_defl"=c(-5000/cap,35000/cap),"yrange"=c(-5000/cap,37000/cap))
scalef <- 1000
...
print("Figure 2 - FLander")
print(npv_plots_list$fl_defl)

print("Figure 2 - Wallonia")
print(npv_plots_list$wa_defl)
